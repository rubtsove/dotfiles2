...
while true; do
  ...
  echo -e "${GREY}нажми ${CYAN}4${GREY} - для установки FZF"
  echo -e "${GREY}нажми ${CYAN}5${GREY} - для удаления старых и установки новых dotfiles + fzf "
  ...
  case "$choose" in
    ...
    4) fzf_install && anykey && source_bashrc && anykey;;
    5) delete_dotfiles && anykey && copy_dotfiles && anykey && fzf_install && anykey && source_bashrc && anykey;;
    0) echo -e "\n${GREY}Пока !!!" && break;;
